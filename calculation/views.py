...
def spent_update(request):
    ...
    # return render(request, 'spent.html', {'spent': list(queryset), 'buildings': buildings})
...
def sorted_spent(request):
    ...
    if request.method == 'POST':
        ...
        if category == 'دستمزد':
            ...
            total_price = Spent.objects.filter(category="نظام مهندسی", building=building).aggregate(total_price=Sum('price'))
            total_price = list(total_price.values())
            try:
                total_price = f"{total_price[0]:,}"
            except:
            ...
            total_price = Spent.objects.filter(category="شهرداری", building=building).aggregate(total_price=Sum('price'))
            total_price = list(total_price.values())
            try:
                total_price = f"{total_price[0]:,}"
            except:
    ...
    return render(request, 'sorted_spent.html', {'sorted_spent': list(queryset), 'total_price': total_price, 'buildings': buildings})
...
def search_spent(request):
    ...
    if request.method == 'POST':
        ...
        total_price = Spent.objects.filter(subject__icontains=search, building=building).aggregate(total_price=Sum('price'))
        total_price = list(total_price.values())
        total_price = f"{total_price[0]:,}"

    return render(request, 'search_spent.html', {'search_spent': list(queryset), 'total_price': total_price, 'buildings': buildings})
...
def generate_excel(request):
    ...
    response['Content-Disposition'] = 'attachment; filename=Building Spent.xlsx'
    wb.save(response)
